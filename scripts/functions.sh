...
openDiskImage() {
    export DISKIMAGE="$(cat config/build | grep ^Name:.* | sed 's/^Name: \(.*\)$/\1/g')-${1}.img"
    ...
    export DEVB=$(losetup -f)
    losetup -o 4194304 --sizelimit $(expr $OFFSET '-' 4194304) $DEVB $DISKIMAGE
    export DEVR=$(losetup -f)
    losetup -o $OFFSET $DEVR $DISKIMAGE
    mkdosfs -n boot -F 32 -v $DEVB
    export DISKUUID="$(dd if=$DISKIMAGE skip=440 bs=1 count=4 2>/dev/null | xxd -e | cut -f 2 -d' ')"
    export ROOTFS=rootfs
    export BOOTFS=bootfs
